...
import { Search, Menu, ShoppingCart, X, Eye, Hand, Brain, Ear, Settings, VolumeX, Volume2 } from "lucide-react"
...
export default function Dashboard() {
  ...
  useEffect(() => {
    ...
    htmlElement.classList.remove("text-base", "text-lg", "text-xl", "high-contrast", "simplified-ui", "reduced-motion")
  ...
  const readDescription = async (text: string) => {
    ...
    try {
        const response = await axios.post('http://192.168.88.216:8000/api/text-to-speech/', { text });
  ...
  const getButtonSize = () => {
    return disabilityType === "motor" || disabilityType === "visual" ? "lg" : "default"
  }
  ...
  return (
    <div
      className={`min-h-screen bg-gradient-to-b from-blue-50 to-white dark:from-slate-900 dark:to-slate-800 dark:text-white ${disabilityType === "visual" ? "text-lg" : ""}`}
    >
      {/* Header */}
      <motion.header
        style={{ opacity: headerOpacity, y: headerY, scale: headerScale }}
        className={`sticky top-0 z-50 bg-white/80 dark:bg-slate-900/80 backdrop-blur-md border-b border-blue-100 dark:border-slate-700 ${disabilityType === "visual" ? "py-4" : ""}`}
      >
        <div className="container mx-auto px-4 py-3 flex items-center justify-between">
            ...
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <Button
                  variant="outline"
                  size={getButtonSize()}
                  className={`ml-2 ${disabilityType !== "none" ? "bg-blue-100 dark:bg-blue-900" : ""}`}
                >
                  {getDisabilityIcon()}
                  <span className="hidden sm:inline">{getDisabilityLabel()}</span>
                </Button>
              </DropdownMenuTrigger>
              <DropdownMenuContent align="start" className="w-56">
                ...
                <DropdownMenuSeparator />
                <DropdownMenuItem
                  className={disabilityType === "none" ? "bg-blue-50 dark:bg-blue-900/30" : ""}
                ...
                <DropdownMenuItem
                  className={disabilityType === "visual" ? "bg-blue-50 dark:bg-blue-900/30" : ""}
                ...
                <DropdownMenuItem
                  className={disabilityType === "motor" ? "bg-blue-50 dark:bg-blue-900/30" : ""}
                ...
                <DropdownMenuItem
                  className={disabilityType === "cognitive" ? "bg-blue-50 dark:bg-blue-900/30" : ""}
                ...
                <DropdownMenuItem
                  className={disabilityType === "hearing" ? "bg-blue-50 dark:bg-blue-900/30" : ""}
          ...
          <div className="hidden md:flex items-center gap-4 flex-1 max-w-md mx-4">
            <div className="relative w-full">
              ...
              <Input
                ...
                className={`pl-10 bg-blue-50 dark:bg-slate-800 border-blue-100 dark:border-slate-700 focus-visible:ring-blue-300 dark:focus-visible:ring-blue-500 ${disabilityType === "visual" ? "text-lg py-6" : disabilityType === "motor" ? "py-5" : ""}`}
              />
            </div>
          </div>
          ...
          <div className="flex items-center gap-3">
            <Button
              variant="ghost"
              size={disabilityType === "motor" || disabilityType === "visual" ? "lg" : "icon"}
            ...
            >
              <ShoppingCart
                className={`${disabilityType === "motor" || disabilityType === "visual" ? "h-5 w-5 mr-2" : "h-5 w-5"}`}
              />
              {(disabilityType === "motor" || disabilityType === "visual") && <span>Panier</span>}
      ...
      <main className={`container mx-auto px-4 py-8 ${getAccessibilityClasses()}`}>
        {/* Hero Section */}
        <motion.div
          ref={heroRef}
          initial="hidden"
          ...
          <div className="grid md:grid-cols-2 gap-8 items-center">
            <div className="space-y-6">
              <motion.h1
                ...
                className={`text-4xl md:text-5xl font-bold text-blue-900 dark:text-blue-100 leading-tight ${disabilityType === "visual" ? "text-5xl md:text-6xl" : ""}`}
              >
              ...
              <motion.p
                variants={heroItemVariants}
                className={`text-lg text-blue-700 dark:text-blue-300 ${disabilityType === "visual" ? "text-xl md:text-2xl" : ""} ${disabilityType === "cognitive" ? "leading-relaxed" : ""}`}
              >
              ...
              <motion.div variants={heroItemVariants} className="flex flex-wrap gap-4">
                <Button
                  size={getButtonSize()}
                  className={`bg-blue-600 hover:bg-blue-700 dark:bg-blue-500 dark:hover:bg-blue-600 text-white ${disabilityType === "motor" ? "py-6 text-lg" : ""}`}
                >
                ...
                <Button
                  size={getButtonSize()}
                  variant="outline"
                  className={`border-blue-300 dark:border-blue-500 text-blue-600 dark:text-blue-300 hover:bg-blue-50 dark:hover:bg-slate-800 ${disabilityType === "motor" ? "py-6 text-lg" : ""}`}
                >
        ...
        <div
          ref={productsRef}
          className={`grid grid-cols-1 ${disabilityType === "motor" || disabilityType === "cognitive" ? "sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-2" : "sm:grid-cols-2 lg:grid-cols-4"} gap-6 ${disabilityType === "motor" ? "gap-8" : ""}`}
        >
          {products.map((product, i) => (
            <motion.div
              ...
              whileHover={{ y: disabilityType === "motor" ? -4 : -8, transition: { duration: 0.3 } }}
              className="group"
              onClick={(e) => handleProductClick(product, e)}
            >
              <div className="cursor-pointer">
                <Card
                  className={`overflow-hidden border-none rounded-xl shadow-md hover:shadow-xl transition-all duration-300 bg-white dark:bg-slate-800 ${disabilityType === "motor" ? "border-2 border-blue-200 dark:border-blue-700" : ""}`}
                >
                  <div className="relative">
                    <div className="aspect-square overflow-hidden bg-gradient-to-br from-blue-50 to-teal-50 dark:from-blue-900/30 dark:to-teal-900/30">
                      <div className="relative w-full h-full">
                        ...
                        />
                        <div
                          className={`absolute top-2 left-2 bg-teal-500 text-white ${disabilityType === "visual" ? "text-base px-3 py-1.5" : "text-xs"} font-medium px-2 py-1 rounded-full`}
                        >
                          Adapté
                        </div>
                      </div>
                    </div>
                    <div className={`p-5 ${disabilityType === "visual" ? "p-6" : ""}`}>
                      <div
                        className={`text-xs text-blue-500 dark:text-blue-300 mb-2 font-medium ${disabilityType === "visual" ? "text-sm" : ""}`}
                      >
                        {product.rewardPoints} Points de fidélité
                      </div>
                      <h3
                        className={`font-medium mb-3 line-clamp-2 ${disabilityType === "visual" ? "text-lg h-14" : "text-sm h-10"} text-blue-900 dark:text-blue-100`}
                      >
                        {product.name}
                      </h3>
                      <div className="flex justify-between items-center">
                        <p
                          className={`font-bold ${disabilityType === "visual" ? "text-2xl" : "text-lg"} text-blue-800 dark:text-blue-200`}
                        >
                          {product.price}
                        </p>
                      </div>
                      <Button
                        className={`w-full mt-4 bg-blue-600 hover:bg-blue-700 dark:bg-blue-500 dark:hover:bg-blue-600 text-white transition-all duration-300 transform group-hover:scale-105 ${disabilityType === "motor" ? "py-6 text-lg mt-6" : ""} ${disabilityType === "visual" ? "text-lg py-5" : ""}`}
                      >
                        {disabilityType === "cognitive" ? "ACHETER" : "ACHETER MAINTENANT"}
                      </Button>
                      <p
                        className={`text-xs text-blue-500 dark:text-blue-400 mt-3 text-center ${disabilityType === "visual" ? "text-sm" : ""}`}
                      >
        ...
        {showPopup && selectedProduct && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50">
            <motion.div
              ...
              className={`relative w-full max-w-2xl bg-white dark:bg-slate-800 rounded-xl shadow-2xl overflow-hidden ${disabilityType === "motor" ? "border-4 border-blue-300 dark:border-blue-700" : ""}`}
            >
              <Button
                variant="ghost"
                size={disabilityType === "motor" || disabilityType === "visual" ? "lg" : "icon"}
                className={`absolute right-2 top-2 z-10 text-blue-600 dark:text-blue-400 hover:bg-blue-100 dark:hover:bg-slate-700 ${disabilityType === "motor" ? "right-4 top-4" : ""}`}
                onClick={closePopup}
              >
                <X className={`${disabilityType === "motor" || disabilityType === "visual" ? "h-6 w-6" : "h-5 w-5"}`} />
                {disabilityType === "motor" && <span className="ml-2">Fermer</span>}
              </Button>

              <div
                className={`grid ${disabilityType === "motor" || disabilityType === "cognitive" ? "grid-cols-1" : "md:grid-cols-2"} gap-0`}
              >
                ...
                <div className={`p-6 ${disabilityType === "visual" ? "p-8" : ""} flex flex-col`}>
                  <div className="mb-2">
                    <div
                      className={`text-xs text-blue-500 dark:text-blue-300 mb-1 font-medium ${disabilityType === "visual" ? "text-base" : ""}`}
                    >
                      {selectedProduct.rewardPoints} Points de fidélité
                    </div>
                    <h2
                      className={`font-bold text-blue-900 dark:text-blue-100 mb-2 ${disabilityType === "visual" ? "text-2xl" : "text-xl"}`}
                    >
                      {selectedProduct.name}
                    </h2>
                    <p
                      className={`font-bold text-blue-800 dark:text-blue-200 mb-4 ${disabilityType === "visual" ? "text-3xl" : "text-2xl"}`}
                    >
                  ...
                  <div className="flex-grow">
                    <div className="flex items-center justify-between mb-2">
                      <h3
                        className={`font-medium text-blue-700 dark:text-blue-300 ${disabilityType === "visual" ? "text-lg" : ""}`}
                      >
                        Description
                      </h3>

                      {/* Audio controls for description */}
                      {disabilityType === "visual" && selectedProduct.description && (
                          ...
                          <Button
                            ...
                            onClick={(e) => {
                              ...
                              if (isPlaying) {
                                ...
                                readDescription(selectedProduct.description || "")
                              }
                            }}
                          >
                            {isPlaying ? (
                    ...
                    <p
                      className={`text-blue-700 dark:text-blue-300 mb-6 ${disabilityType === "visual" ? "text-lg" : ""} ${disabilityType === "cognitive" ? "leading-relaxed" : ""}`}
                    >
                  ...
                  <div className="mt-auto space-y-4">
                    <Button
                      className={`w-full bg-blue-600 hover:bg-blue-700 dark:bg-blue-500 dark:hover:bg-blue-600 text-white ${disabilityType === "motor" ? "py-6 text-lg" : ""} ${disabilityType === "visual" ? "text-lg py-5" : ""}`}
                    >
                      {disabilityType === "cognitive" ? "ACHETER" : "AJOUTER AU PANIER"}
                    </Button>
                    <p
                      className={`text-xs text-blue-500 dark:text-blue-400 text-center ${disabilityType === "visual" ? "text-sm" : ""}`}
                    >
